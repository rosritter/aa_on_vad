...
import torch
from src.utils.audio_utils import vad_forward, remove_silence, remove_silence_from_beginning
...
class LibriSpeechWrapper(Dataset):
    def __init__(self, 
                 librispeech_dataset, 
                 sample_rate=16000,
                 remove_silence_on_edges:bool=False, 
                 erase_silence:bool=False, 
        ...
        self.remove_silence_on_edges = remove_silence_on_edges
    ...
    def __getitem__(self, idx):
        ...
        if self.remove_silence_on_edges:
            waveform = remove_silence_from_beginning(torch.flip(waveform, dims=[-1]), 16000, step_duration=0.02, energy_threshold=0.01)
            waveform = remove_silence_from_beginning(torch.flip(waveform, dims=[-1]), 16000, step_duration=0.02, energy_threshold=0.01)
